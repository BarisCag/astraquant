...
pub fn export_experiment_json<W: Write>(
...
pub fn export_experiment_csv<W: Write>(
    suite: &ExperimentSuite,
    out: &mut W,
) -> std::io::Result<()> {
...
pub fn export_experiment_mermaid_topology<W: Write>(
    suite: &ExperimentSuite,
    out: &mut W,
) -> std::io::Result<()> {
    ...
    for run in &suite.runs {
        ...
        writeln!(
            out,
            "    Run_{} --> Hash_{}[{}]",
            run.run_id, run.run_id, run.final_state_hash
        )?;
    }
    Ok(())
}

pub fn export_replay_certification_manifest<W: Write>(
    run: &ExperimentRun,
    out: &mut W,
) -> std::io::Result<()> {
