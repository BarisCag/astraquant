...
pub enum SweepDimension {
    LiquidityCollapseSeverity { start_ppm: u64, end_ppm: u64, steps: u64 },
    MarginMultiplier { start_ppm: u64, end_ppm: u64, steps: u64 },
    VenueLatency { start_seq: u64, end_seq: u64, steps: u64 },
    CollateralHaircut { start_ppm: u64, end_ppm: u64, steps: u64 },
    StressSeverity { start_ppm: u64, end_ppm: u64, steps: u64 },
    RecoveryDelay { start_seq: u64, end_seq: u64, steps: u64 },
}
    ...
    pub fn generate_plan(&self, base_parameters: &ExperimentParameterSet) -> SweepExecutionPlan {
        ...
        for dimension in &self.dimensions {
            ...
            for base_set in &parameter_sets {
                match dimension {
                    SweepDimension::LiquidityCollapseSeverity { start_ppm, end_ppm, steps } => {
                        let step_size = if *steps > 1 { (end_ppm - start_ppm) / (steps - 1) } else { 0 };
                        for i in 0..*steps {
                    ...
                    SweepDimension::MarginMultiplier { start_ppm, end_ppm, steps } => {
                        let step_size = if *steps > 1 { (end_ppm - start_ppm) / (steps - 1) } else { 0 };
                        for i in 0..*steps {
                            let mut new_set = base_set.clone();
                    ...
                    SweepDimension::VenueLatency { start_seq, end_seq, steps } => {
                        let step_size = if *steps > 1 { (end_seq - start_seq) / (steps - 1) } else { 0 };
                        for i in 0..*steps {
                            let mut new_set = base_set.clone();
                    ...
                    SweepDimension::CollateralHaircut { start_ppm, end_ppm, steps } => {
                        let step_size = if *steps > 1 { (end_ppm - start_ppm) / (steps - 1) } else { 0 };
                        for i in 0..*steps {
                            let mut new_set = base_set.clone();
                    ...
                    SweepDimension::StressSeverity { start_ppm, end_ppm, steps } => {
                        let step_size = if *steps > 1 { (end_ppm - start_ppm) / (steps - 1) } else { 0 };
                        for i in 0..*steps {
                            let mut new_set = base_set.clone();
                    ...
                    SweepDimension::RecoveryDelay { start_seq, end_seq, steps } => {
                        let step_size = if *steps > 1 { (end_seq - start_seq) / (steps - 1) } else { 0 };
                        for i in 0..*steps {
                            let mut new_set = base_set.clone();
