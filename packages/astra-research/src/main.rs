...
fn main() {
    ...
    let datasets = vec![
        (
            ...
            "2010 Flash Crash",
            "2010-05-06",
            build_flash_crash_events(),
        ),
        (
            "lehman_2008",
            "2008 Lehman Collapse",
            "2008-09-15",
            build_lehman_collapse_events(),
        ),
        (
            "covid_2020",
            "2020 COVID Crash",
            "2020-03-16",
            build_covid_crash_events(),
        ),
    ];
    ...
    for (id, name, date, events) in &datasets {
        ...
        let _ = r3.run(&dataset, CHECKPOINT_INTERVAL);
        ...
        .unwrap();
        println!(
            "  -> Generated {}_merkle_audit.json ({} roots)",
            id,
            merkle.len()
        );
        ...
        let interventions = vec![
            ...
            (
                InterventionType::ShortSellingBan {
    ...
    fs::write(
        format!("{}/counterfactual_matrix.json", out_dir),
        matrix_json,
    )
    .unwrap();
    println!(
        "  -> Generated counterfactual_matrix.json ({} rows)",
        matrix.len()
    );
    ...
    for (id, _name, _date, events) in &datasets {
        ...
        println!(
            "  -> {}: Best Fit [Herding: {}, Loss Aversion: {}] (Error: {:.2}%)",
            id, res.best_herding_factor, res.best_loss_aversion, res.cascade_depth_error_pct
        );

        let seed = astra_core::events::BehavioralSeed::new(
            res.best_herding_factor,
            res.best_loss_aversion,
            0.5,
            0.8,
            42,
        );
        ...
        let interventions = vec![
            (
                astra_research::counterfactual::InterventionType::CircuitBreakerHalt {
                    duration: 60,
                },
                400,
            ),
            (
                astra_research::counterfactual::InterventionType::LiquidityInjection,
                350,
            ),
            (
                astra_research::counterfactual::InterventionType::ShortSellingBan {
    ...
    fs::write(
        format!("{}/behavioral_counterfactual_delta.json", out_dir),
        behave_json,
    )
    .unwrap();
    println!(
        "  -> Generated behavioral_counterfactual_delta.json ({} rows)",
        behavioral_deltas.len()
    );
