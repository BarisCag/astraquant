...
pub struct PanicLiquidationSpiralScenario {
    pub seed: u64,
    pub severity: ScenarioSeverity,
    pub activation_windows: Vec<(u64, u64)>,
}

impl ScenarioDefinition for PanicLiquidationSpiralScenario {
    fn scenario_id(&self) -> &'static str {
        "panic_liquidation_spiral"
    }
        ...
        for (start, _end) in &self.activation_windows {
            if current_sequence == *start {
                events.push(AstraEvent {
                    ...
                    payload: serialize_canonical(&"PanicLiquidationSpiral").unwrap(),
