...
pub struct MarginSpiralScenario {
    pub seed: u64,
    pub severity: ScenarioSeverity,
    pub activation_windows: Vec<(u64, u64)>,
}

impl ScenarioDefinition for MarginSpiralScenario {
    fn scenario_id(&self) -> &'static str { "margin_spiral" }
            ...
            if current_sequence >= *start && current_sequence <= *end {
                if lcg.next_bool_ppm(50_000) {
                    events.push(AstraEvent {
                        ...
                        payload: serialize_canonical(&self.severity.margin_requirement_multiplier).unwrap(),
