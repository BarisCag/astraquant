...
pub struct CrossVenueArbitrageBreakdownScenario {
    pub seed: u64,
    pub severity: ScenarioSeverity,
    pub activation_windows: Vec<(u64, u64)>,
}

impl ScenarioDefinition for CrossVenueArbitrageBreakdownScenario {
    fn scenario_id(&self) -> &'static str {
        "cross_venue_arbitrage_breakdown"
    }
        ...
        for (start, end) in &self.activation_windows {
            if current_sequence == *start {
                events.push(AstraEvent {
                    ...
                    payload: serialize_canonical(&"CrossVenueArbitrageBreakdown").unwrap(),
