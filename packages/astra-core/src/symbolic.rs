...
mod tests {
    ...
    fn test_symbolic_divergence_detection() {
        ...
        let mut kernel_a = AstraKernel::new(StrategyRuntime::new(ExchangeRuntime::new(limits.clone())));
        let mut kernel_b = AstraKernel::new(StrategyRuntime::new(ExchangeRuntime::new(limits)));

        // Create some events
        let event1 = AstraEvent::new(1, 1, EventType::MarketTick, vec![], PayloadMetadata::new(PayloadEncoding::RawBytes, 1));
        ...
        let diverged = SymbolicReplayEngine::detect_divergence(&mut kernel_a, &mut kernel_b, &events);
        assert!(diverged, "SymbolicReplayEngine failed to detect divergence between two distinct kernel states");
    }
}
